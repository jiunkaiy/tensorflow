...
TEST(TensorWrapperTest, SanityTest) {
  ...
  EXPECT_EQ(tensor_wrapper.GetStaticTensorData(), nullptr);
  std::vector<std::uint8_t> data = {1, 2, 3};
  void* data_ptr = reinterpret_cast<void*>(data.data());
  // expect no use, since tensor type not correct
  tensor_wrapper.SetTensorData(data.size(), data_ptr);
  EXPECT_EQ(tensor_wrapper.GetStaticTensorData(), nullptr);
}
...
TEST(TensorWrapperTest, CopyTensorTest) {
  ...
  TensorWrapper tensor_wrapper{0, QNN_TENSOR_TYPE_STATIC,
                               ...
                               QuantizeParamsWrapperVariant(), dummy_dims};
  ...
  EXPECT_EQ(copied.GetStaticTensorData(), nullptr);
  std::vector<std::uint8_t> data = {1, 2, 3};
  void* data_ptr = reinterpret_cast<void*>(data.data());
  copied.SetTensorData(data.size(), data_ptr);
  const auto* tensor_data =
      reinterpret_cast<const std::uint8_t*>(copied.GetStaticTensorData());
...
TEST(TensorWrapperTest, MoveTensorTest) {
  ...
  const auto* tensor_data =
      reinterpret_cast<const std::uint8_t*>(moved.GetStaticTensorData());
  for (size_t i = 0; i < data.size(); i++) {
    EXPECT_EQ(tensor_data[i], data[i]);
  }
}
