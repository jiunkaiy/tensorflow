...
std::optional<absl::Span<const T>> TensorWrapper::GetStaticTensorData() const {
  if (GetDataType() != GetQnnDataType<T>(IsQuant())) {
    std::cout << "ERROR: GetStaticTensorData() with incorrect template type."
              << std::endl;
    return std::nullopt;
  }
  ...
  if (qnn_tensor_.v2.clientBuf.dataSize == 0 ||
    ...
    std::cout << "ERROR: Empty StaticTensorData." << std::endl;
    return std::nullopt;
  }

  if (qnn_tensor_.v2.clientBuf.dataSize != GetTensorSize()) {
    std::cout << "ERROR: Tensor size inconsistent with stored data."
              << std::endl;
    return std::nullopt;
  }
  ...
  if (!num_elements) {
    std::cout << "ERROR: No element in this tensor." << std::endl;
    return std::nullopt;
  }
