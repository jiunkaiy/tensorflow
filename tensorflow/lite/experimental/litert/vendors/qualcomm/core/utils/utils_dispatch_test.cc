...
// TODO: implement with QC dispatch flow or litert dispatch flow
TEST(DispatchDelegate, inferenceCtx) {
#if !defined(__ANDROID__)
  GTEST_SKIP()
      << "This test is specific to Android devices with a Qualcomm NPU";
#endif

  ASSERT_NE(model_name, "");
  ...
  auto model = litert::internal::LoadBinaryFile(model_name);
  EXPECT_TRUE(model) << model.Error();
  ABSL_LOG(INFO) << "Loaded model " << model_name << ", " << model->Size()
                 << " bytes";
