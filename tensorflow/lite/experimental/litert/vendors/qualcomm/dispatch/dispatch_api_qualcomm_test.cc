...
TEST(Qualcomm, DispatchApiWithDmaBufInt16Model) {
  ...
  std::vector<std::int16_t> raw_input_tensor_0 = {
      static_cast<std::int16_t>(0.5 / kInputScale),
      static_cast<std::int16_t>(0.5 / kInputScale)};
  std::vector<std::int16_t> raw_input_tensor_1 = {
      static_cast<std::int16_t>(0.5 / kInputScale),
      static_cast<std::int16_t>(0.5 / kInputScale)};
  std::vector<std::int16_t> raw_output_tensor_0 = {
      static_cast<std::int16_t>(1 / kOutputScale),
      static_cast<std::int16_t>(1 / kOutputScale)};
  std::vector<float> golden_output_tensor_0 = {1.0, 1.0};

  std::vector<std::uint16_t> input_tensor_0;
  std::vector<std::uint16_t> input_tensor_1;
  std::vector<std::uint16_t> output_tensor_0;

  qnn::ConvertDataFromInt16toUInt16(raw_input_tensor_0, input_tensor_0);
  qnn::ConvertDataFromInt16toUInt16(raw_input_tensor_1, input_tensor_1);
  qnn::ConvertDataFromInt16toUInt16(raw_output_tensor_0, output_tensor_0);
  ...
  auto model_file_name = litert::testing::GetTestFilePath("add_uint16.bin");
  auto model = litert::internal::LoadBinaryFile(model_file_name);
  EXPECT_TRUE(model) << model.Error();
  ABSL_LOG(INFO) << "Loaded model " << model_file_name << ", " << model->Size()
                 << " bytes";
...

  {
    ...
    ASSERT_EQ(LiteRtLockTensorBuffer(output_tensor_buffer, &host_mem_addr),
              kLiteRtStatusOk);
    auto output =
        absl::MakeSpan(static_cast<const std::uint16_t*>(host_mem_addr),
                       output_tensor_0.size());

    std::vector<std::uint16_t> uint16_output(output.begin(), output.end());

    std::vector<std::int16_t> int16_output;
    qnn::ConvertDataFromUInt16toInt16(uint16_output, int16_output);
    std::vector<float> dequant_output;
    for (const auto& data : int16_output) {
      dequant_output.emplace_back(data * kOutputScale);
    }

    for (auto i = 0; i < golden_output_tensor_0.size(); ++i) {
      ABSL_LOG(INFO) << dequant_output[i] << "\t" << golden_output_tensor_0[i];
    }
    EXPECT_THAT(dequant_output,
                Pointwise(testing::FloatNear(1e-3), golden_output_tensor_0));
    ASSERT_EQ(LiteRtUnlockTensorBuffer(output_tensor_buffer), kLiteRtStatusOk);
  }
